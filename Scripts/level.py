...
class Level:
    ...
    def create_jump_particles(self, pos):
        if self.player_tiles.sprite.facing_right:
    ...
    def get_player_on_ground(self):
        if self.player_tiles.sprite.on_ground:
            self.player_on_ground = True
        else:
            self.player_on_ground = False

    def create_landing_dust(self):
        if not self.player_on_ground and self.player_tiles.sprite.on_ground and not self.dust_sprite.sprites():
            if self.player_tiles.sprite.facing_right:
                offset = pygame.math.Vector2(10,15)
            else:
                offset = pygame.math.Vector2(-10, 15)
            fall_dust_particle = ParticleEffect(self.player_tiles.sprite.rect.midbottom - offset, 'land')
